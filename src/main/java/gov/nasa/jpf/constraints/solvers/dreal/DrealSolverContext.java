...
public class DrealSolverContext extends SolverContext {

  private final DrealSolver dreal;
  private Deque<List<Expression<Boolean>>> exprStack = new ArrayDeque<List<Expression<Boolean>>>();
  
  public DrealSolverContext(DrealSolver solver) {
    this.dreal = solver;
  ...
  public Result solve(Valuation val) {
    ...
    Expression<Boolean> expr = combineDeque(exprStack);
    return this.dreal.solve(expr, val);
  }
